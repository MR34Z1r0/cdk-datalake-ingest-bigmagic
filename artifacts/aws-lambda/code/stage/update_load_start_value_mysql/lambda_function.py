...
def lambda_handler(event, context):
    ...
    try:
        ...
        test_item = read_table_metadata(dynamodb_table, 'TARGET_TABLE_NAME', test_value)
        credentials = read_table_metadata(dynamodb_credentials, 'ENDPOINT_NAME', test_item['ENDPOINT'])
        ...
        src_db_port_number = int(credentials["DB_PORT_NUMBER"])

        for try_number in range(3):
            try:
                logger.info(f"try {try_number+1}....")
                connection = pymysql.connect(host=src_server_name, user=user_name, password=password, database=src_db_name, port=src_db_port_number)
        ...
        for table in dynamodb_key:
            try:
                item = read_table_metadata(dynamodb_table, 'TARGET_TABLE_NAME', table)
                if item['FILTER_OPERATOR'].strip() in ['between-date']:
                    if ',' in item['FILTER_COLUMN']:
                        filter = item['FILTER_COLUMN'].split(',')
                        query = f"SELECT GREATEST(MAX({filter[0]}),MAX({filter[1]})) FROM {item['SOURCE_SCHEMA']}.{item['SOURCE_TABLE']}"
                    else:
                        query = "SELECT max(" + item['FILTER_COLUMN'] + ") FROM " + item['SOURCE_SCHEMA'] + "." + item['SOURCE_TABLE']
                    logger.info(f"executing query: {query}")
                    cursor.execute(query)
                    result = cursor.fetchone()[0]
                    logger.info("DynamoDb Metadata Record Found! and it´s mysql!")
                    end_value = str(item['END_VALUE']).strip()
                    logger.info("Updating start and end value")
                    start_value = end_value
                    end_value = str(result).strip()
                    if 'FILTER_TYPE' in item.keys() and item['FILTER_TYPE'] in ['BIGINT']:
                        end_value = datetime.fromtimestamp(int(end_value))
                    end_value = is_valid_date(str(end_value))
                ...
                aux.remove(table)
                update_attribute_value_dyndb('TARGET_TABLE_NAME', table, 'FAIL REASON', f"str(e)", dynamodb_table)
                update_attribute_value_dyndb('TARGET_TABLE_NAME', table, 'STATUS_RAW', 'FAILED', dynamodb_table)

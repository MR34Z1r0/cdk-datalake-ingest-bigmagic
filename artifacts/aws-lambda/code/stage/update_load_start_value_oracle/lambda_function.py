...
def Oracle_Connection(src_db_name, username, password):
    connection = cx_Oracle.connect(username, password, src_db_name)
...
def lambda_handler(event, context):
    ...
    try:
        ...
        test_item = read_table_meta_data(dynamodb_table, 'TARGET_TABLE_NAME', test_value)
        credentials = read_table_meta_data(dynamodb_credentials, 'ENDPOINT_NAME', test_item['ENDPOINT'])
        ...
        connect_url = f"{src_server_name}:{str(src_db_port_number)}/{src_db_name}"

        for try_number in range(3):
            try:
                logger.info(f"try {try_number+1}....")
                connection = Oracle_Connection(connect_url, user_name, password)
                break
            except Exception as e:
                logger.error(str(e))
                if try_number < 2:
        ...
        for table in dynamodb_key:
            try:
                item = read_table_meta_data(dynamodb_table, 'TARGET_TABLE_NAME', table)
                if item['FILTER_OPERATOR'].strip() in ['between-date']:
                    ...
                    logger.info("DynamoDb Metadata Record Found! and it´s Oracle!")
                    logger.info('Executing Query on database: {}'.format(query))

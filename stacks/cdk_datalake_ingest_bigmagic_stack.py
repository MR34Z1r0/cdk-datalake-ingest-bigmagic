...
class CdkDatalakeIngestBigmagicStack(Stack):
    def __init__(self, scope: Construct, construct_id: str, project_config, **kwargs):
    ...
    def _create_iam_roles(self):
        ...
        self.role_light_transform = self.builder.build_light_transform_role(
            f"{self.PROJECT_CONFIG.app_config['datasource'].lower()}_light_transform",
            resources,
            light_transform_tags
        )
        
        self.role_crawler = self.builder.build_crawler_role(
            f"{self.PROJECT_CONFIG.app_config['datasource'].lower()}_crawler",
            resources,
            crawler_tags
        )
        
        self.role_crawler.add_to_principal_policy(
            iam.PolicyStatement(
                effect=iam.Effect.ALLOW,
                actions=["iam:PassRole"],
                resources=[self.role_crawler.role_arn]
            )
        )
        
        self.role_step_function = self.builder.build_step_function_role(
            f"{self.PROJECT_CONFIG.app_config['datasource'].lower()}_step_function",
            resources,
            step_function_tags
        )
    ...
    def _create_outputs(self):
        ...
        # Output Glue connection names for cross-stack reference
        for connection_name, connection in self.glue_connections.items():
            # Create a clean output name following CloudFormation naming conventions
            clean_name = connection_name.replace('-', '').replace('_', '').title()
            output_name = f"GlueConnection{clean_name}Name"
            CfnOutput(self, output_name, value=connection.connection_name)
        ...
        catalog_job_config = GlueJobConfig(
            ...
            executable=glue.JobExecutable.python_shell(
                ...
                script=glue.Code.from_bucket(
                    ...
                    f"{self.Paths.AWS_ARTIFACTS_GLUE_CODE_STAGE}/crawler_catalog.py"
                )
            ),
            default_arguments={
        ...
        for endpoint_name in endpoint_names:
            ...
            crawler_job_config = GlueJobConfig(
                ...
                executable=glue.JobExecutable.python_shell(
                    ...
                    script=glue.Code.from_bucket(
                        self.s3_artifacts_bucket,
                        f"{self.Paths.AWS_ARTIFACTS_GLUE_CODE_STAGE}/crawler_stage.py"
                    )
                ),
                default_arguments={
                    '--S3_STAGE_BUCKET': self.s3_stage_bucket.bucket_name,
    ...
    def _get_resource_arns(self):
        ...
        resources = {
            ...
            'dynamodb': [
                self.dynamodb_logs_table.table_arn
            ],
            'sns': [
            ...
            'iam': [
                f"arn:aws:iam::{self.PROJECT_CONFIG.account_id}:role/{self.PROJECT_CONFIG.project_name}-{self.PROJECT_CONFIG.environment.value.lower()}-datalake-*"
            ]
        }
        return resources
    ...
    def _create_base_step_function(self):
        start_job = tasks.CallAwsService(
            ...
            action="startJobRun",
            parameters={
                "JobName": sfn.JsonPath.string_at("$.job_name"),
                "Arguments": sfn.JsonPath.object_at("$.job_arguments")
            },
            iam_resources=["*"],
            result_path="$.job_run"
        )
        ...
        check_job_status = tasks.CallAwsService(
            ...
            parameters={
                "JobName": sfn.JsonPath.string_at("$.job_name"),
                "RunId": sfn.JsonPath.string_at("$.job_run.JobRunId")
            },
        ...
        job_status_choice.when(
            sfn.Condition.string_equals("$.job_status.JobRun.JobRunState", "SUCCEEDED"),
            notify_success.next(job_succeeded)
        ).when(
            sfn.Condition.string_equals("$.job_status.JobRun.JobRunState", "FAILED"),
            job_failed_chain
        ).when(
            sfn.Condition.string_equals("$.job_status.JobRun.JobRunState", "STOPPED"),
            job_stopped_chain
        ).when(
            sfn.Condition.string_equals("$.job_status.JobRun.JobRunState", "TIMEOUT"),
            job_timeout_chain
        ).otherwise(
        ...
        CfnOutput(
            self, "BaseStepFunctionArn", 
            value=self.base_step_function.state_machine_arn,
            description="ARN of the base step function that executes Glue jobs"
        )
